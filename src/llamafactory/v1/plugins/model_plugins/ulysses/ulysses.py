...
class UlyssesAttention(torch.nn.Module):
    ...
    def forward(
        ...
        *args: Any
        ...
        sp_world_size = dist.get_world_size(self.spg)
        local_attention_mask_list = [torch.zeros_like(attention_mask) for _ in range(sp_world_size)]
        dist.all_gather(local_attention_mask_list, attention_mask, group=self.spg)
        attention_mask = torch.cat(local_attention_mask_list, dim=1).contiguous()
