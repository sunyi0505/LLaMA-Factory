...
if __name__ == "__main__": # for testing torchrun --nproc_per_node=2 sequence_parallel.py
    ...
    if model_args.sequence_parallel_size == 1:
        input_ids = torch.tensor([[1, 1, 1, 2, 2, 2]])
        ...
        SequenceParallelLossPlugin('compute_loss')(model, inputs)
    else:
        ...
        if rank == 0:
            ...
            position_ids = torch.tensor([[1, 2, 3]])
        else:
            input_ids = torch.tensor([[2, 2, 2]])
            attention_mask = torch.tensor([[1, 1, 1]])
            labels = torch.tensor([[2, 2, -100]])
            position_ids = torch.tensor([[4, 5, 6]])
        inputs = {"input_ids": input_ids.to(rank), "attention_mask": attention_mask.to(rank), "labels": labels.to(rank), "position_ids": position_ids.to(rank)}
        SequenceParallelLossPlugin('sequence_parallel_loss')(model, inputs)
